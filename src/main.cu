...
void addition() {
    ...
    // Declare pointers on the CPU memoru for the vectors
    // to be added (A and B) and the vector holding the result
    int* A;
    int* B;
    int* C;

    // Declare pointers on the CPU memory to hold GPU addresses 
    // of the two vectors to be added.
    int* device_A;
    int* device_B;
    int* device_C;

    // Allocate memory on the CPU memory heap for the tow 
    // vectors to be added
    A = (int *)malloc(DIM * sizeof(int));
    B = (int *)malloc(DIM * sizeof(int));
    C = (int *)malloc(DIM * sizeof(int));

    // Allocate memory on the GPU memory heap for the tow 
    // vectors to be added
    cudaMalloc((void **)&device_A, DIM * sizeof(int));
    cudaMalloc((void **)&device_B, DIM * sizeof(int));
    cudaMalloc((void **)&device_C, DIM * sizeof(int));

    // Initialize the vectors to be added in the CPU memory
    for (int i = 0; i < DIM; i++) {
        A[i] = i;
        B[i] = DIM - i;
    }

    // Copy the vectors to the GPU memory
